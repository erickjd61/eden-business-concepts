    ...
    <svg viewBox="0 0 820 430" className="fw-graphic ps-curve" role="img"
      ...
      <defs>
        <linearGradient id="ascFill" x1="0" y1="0" x2="0" y2="1">
          <stop offset="0%" stopColor="#DBA63E" stopOpacity="0.34" />
          <stop offset="100%" stopColor="#DBA63E" stopOpacity="0" />
        </linearGradient>
        <linearGradient id="descFill" x1="0" y1="0" x2="0" y2="1">
          <stop offset="0%" stopColor="#8A7C5C" stopOpacity="0.20" />
          <stop offset="100%" stopColor="#8A7C5C" stopOpacity="0" />
        </linearGradient>
      </defs>

      {/* target zone behind peak */}
      <rect x="368" y="56" width="84" height="284" fill="#DBA63E" opacity="0.06" />
      {/* baseline */}
      <line x1="40" y1="340" x2="780" y2="340" stroke="rgba(244,238,225,.16)" strokeWidth="1" />
      ...
      {/* strokes */}
      <path d="M70,300 C130,260 150,210 183,196 C240,175 265,158 296,150 C350,138 385,100 410,82" fill="none" stroke="#DBA63E" strokeWidth="2.5" strokeLinecap="round" />
      <path d="M410,82 C435,100 470,138 523,150 C555,158 600,205 636,240 C670,272 710,300 750,318" fill="none" stroke="#8A7C5C" strokeWidth="2.5" strokeLinecap="round" />

      {/* dashed guide at peak */}
      <line x1="410" y1="92" x2="410" y2="340" stroke="#DBA63E" strokeWidth="1" strokeDasharray="3 4" opacity="0.5" />
      ...
      {stages.map(([num, , x], i) => {
        ...
        return (
          <g key={num}>
            {isPeak && <circle cx={x} cy={ys[i]} r="11" fill="none" stroke="#DBA63E" strokeWidth="1.5" opacity="0.5" />}
            <circle cx={x} cy={ys[i]} r={isPeak ? 6 : 4} fill={i <= 3 ? '#DBA63E' : '#8A7C5C'} />
          </g>
        )
      })}
      ...
      <g fontFamily="'Space Mono', monospace">
        {stages.map(([num, label, x], i) => (
          <g key={num} textAnchor="middle">
            <text x={x} y="372" fontSize="11" fontWeight={i === 3 ? 700 : 400} fill={i === 3 ? '#DBA63E' : (i < 3 ? '#CFC6B2' : '#8A7C5C')}>{label}</text>
          </g>
        ))}
        {/* ascent / descent hints */}
        <text x="200" y="404" fontSize="10" fill="#CFC6B2" letterSpacing="2" textAnchor="middle">⟵ BUILDING</text>
        <text x="620" y="404" fontSize="10" fill="#8A7C5C" letterSpacing="2" textAnchor="middle">DRIFTING ⟶</text>
      </g>
    </svg>
  )
}
